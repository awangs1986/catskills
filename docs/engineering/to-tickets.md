...
`to-tickets` publishes into a tracker, so [setup-matt-pocock-skills](setup-matt-pocock-skills.md) must have configured one for this repo, along with the triage-label vocabulary. Either kind works: a real tracker like GitHub or Linear, or local markdown files under `.scratch/`, which is supported out of the box.
...
Upstream is [to-spec](to-spec.md), which hands it a settled spec to slice against; keep both in one unbroken context window. Downstream is [implement](implement.md), which builds one ticket per fresh session, driving [tdd](tdd.md) for the tests and closing with [code-review](code-review.md). When you're unsure which skill or flow fits, [ask-matt](ask-matt.md) routes you.
