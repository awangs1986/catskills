...
Step 1 depends on `docs/agents/issue-tracker.md`, which [setup-matt-pocock-skills](setup-matt-pocock-skills.md) writes. Without it the axis still works if you hand it a path. With no spec at all, the Spec sub-agent is skipped and the report says "no spec available" rather than inventing requirements.
